...
Optional (and the default): keep "read/write text file" capabilities disabled so the agent cannot read or modify files via the ACP client. You can opt back in (for trusted setups) by setting:
...
- `agent-shell-save-summary` will ask the agent (using `agent-shell-summary-prompt`) to generate a concise session summary and save it per project under =$CODEX_HOME/session-summaries=. Use a prefix arg to enter a manual summary instead.
- `agent-shell-list-summaries` lists saved summaries for the current project.
- `agent-shell-use-summary` inserts a saved summary into your compose buffer (helpful when starting a new session).
- Customizations: `agent-shell-summary-storage-enabled`, `agent-shell-summary-directory`, `agent-shell-summary-max-entries`, `agent-shell-summary-max-age-days`, and `agent-shell-summary-prompt`.
- Only the short summary text plus metadata (cwd/model/timestamp) is stored; transcripts remain separate under =.agent-shell/transcripts/=.
...
|                 | agent-shell-save-summary                                | Generate (or manually enter) and save a short session summary.                |
|                 | agent-shell-list-summaries                              | List saved summaries for the current project.                                 |
|                 | agent-shell-use-summary                                 | Insert a saved summary into the current prompt buffer.                        |
